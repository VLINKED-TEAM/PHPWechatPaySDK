...
class Random
{
    ...
    public static function generate($length = 10, $charlist = '0-9a-z')
    {
        ...
        if ($length < 1) {
            throw new InvalidArgumentTypeException('Length must be greater than zero.');
        } elseif ($chLen < 2) {
            throw new InvalidArgumentTypeException('Character list must contain as least two chars.');
        }
        $res = '';
